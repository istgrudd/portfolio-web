...
approach: |
  ...
  The pipeline runs end to end. PyMuPDF extracts and normalizes text from uploaded PDFs; an IndoBERT NER model detects and masks identity attributes (name, contact, institution, photo) to enforce blind screening; a LangChain RAG pipeline retrieves the recruiter-defined qualification rubric for the target role; and DeepSeek V4 scores the candidate per competency dimension, grounding every score in specific evidence from the CV.
